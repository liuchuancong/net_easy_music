...
import 'singerSongItem.dart';

class SingerSongList extends StatefulWidget {
  final String id;
...
class _SingerSongState extends State<SingerSongList> {
  ...
  _buildSongsList() async {
    final Response response = await HttpManager()
            .get(apiList['SINGER_SONG'], data: {
          'id': widget.id,
          '_p': getPlatformPara()
        });
    List<String> idsArray = [];
    print(response.data);
    // await _getAudioPlaylist(widget.id);
  }
